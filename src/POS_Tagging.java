...
public class POS_Tagging {
	...
	ClassNotFoundException {
		...
		File folder = new File("justTexts"); // Magic values, Konstanten verweden!
		File[] listOfFiles = folder.listFiles();

		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile()) {
				// Input of a test file
                            String input = new String(Files.readAllBytes(Paths.get("justTexts/"+listOfFiles[i].getName())), "UTF-8");// Fest codierte Pfade!!!
				// The tagged string
